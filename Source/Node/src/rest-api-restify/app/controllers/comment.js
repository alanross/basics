...
exports.getComment = function( req, res )
{
	Article.findOne( { "comments._id": new ObjectId( req.params.id ) }, { "comments.$": 1 }, function( err, comment )
	{
		...
		else
		{
			if( comment )
			{
				res.json( { type: true, data: new Comment( comment.comments[ 0 ] ) } );
			}
			else
			{
				res.json( { type: false, data: "Comment: " + req.params.id + " not found" } );
			}
		}
	} )
};
...
exports.updateComment = function( req, res, next )
{
	...
	Article.update(
			...
			function( err )
			{
				...
				{
					res.json( { type: true, data: "Comment: " + req.params.id + " updated" } );
				}
			} )
};
